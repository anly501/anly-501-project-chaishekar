...
ggplot(age_chart_data, aes(x=Age, y=Total, color=Age)) +
  ...
  labs(title = "Victims Age Group Distribution", x = "Age", y="Count")+
...
ggplot(gender_chart_data, aes(ymax=ymax, ymin=ymin, xmax=4, xmin=3, fill=Gender)) +
  ...
  theme(text=element_text(size=25))+
  labs(title = "Victimis Gender Distribution")+
...
ggplot(data=race_chart_data, aes(x=Race, y=Total, group=1)) +
  ...
  theme(plot.title = element_text(hjust = 0.5))+
  theme(text=element_text(size=25))+
  labs(title = "Victimis Race Distribution")

victims_data = read_csv("victims_crime_data_yearly.csv")
